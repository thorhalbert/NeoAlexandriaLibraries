...
namespace Mounter
{
    public class MemoryFileSystem : FuseFileSystemBase
    {
        ...
        {
            if (debug) Console.WriteLine($"MEM::GetAttr({RawDirs.HR(path)})");
        ...
        {
            if (debug) Console.WriteLine($"MEM::ReadDir({RawDirs.HR(path)},{offset},{flags})");
            Entry entry = _root.FindEntry(path);
            if (entry == null)
            {
                return -ENOENT;
